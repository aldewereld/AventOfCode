...
class Computer:
    def __init__(self, input1: int, input2: int):
        ...
        while not self.halt:
            if self.memory[self.counter] == 99:
                self.halt = True
                continue
            instruction = self.memory[self.counter:self.counter+4]
            self.decodeAndExecute(instruction)
            self.counter += 4

    def decodeAndExecute(self, instruction):
        op1 = self.memory[int(instruction[1])]
        op2 = self.memory[int(instruction[2])]
        accum = int(instruction[3])
        if instruction[0] == 1:
            #Add
            self.memory[accum] = op1 + op2
